...
module Neurolibre
class ZenodoUploadMystResponder < Responder

  keyname :neurolibre_zenodo_upload_myst

  def define_listening
    required_params :external_call
    @event_action = "issue_comment.created"
    @event_regex = /\A@#{bot_name} zenodo upload myst\.?\s*$/i
  end
  ...
  def default_description
    "Upload the latest MyST article to Zenodo for archival."
  end

  def default_example_invocation
    "@#{bot_name} zenodo upload myst"
  end
end
end