...
module Neurolibre
class BinderBuildResponder < Responder

  keyname :neurolibre_binder_build

  def define_listening
    required_params :external_call
    @event_action = "issue_comment.created"
    @event_regex = /\A@#{bot_name} production build runtime\.?\s*$/i
  end
  ...
  def default_example_invocation
    "@#{bot_name} production build runtime"
  end
end
end