...
module Neurolibre
class BuildBookResponder < Responder
...
# context is inferred and locals are needed for auth etc.
  keyname :neurolibre_build_book

  def define_listening
    required_params :data_from_issue
    @event_action = "issue_comment.created"
    @event_regex = /\A@#{bot_name} build book\.?\s*$/i
  end
  ...
  def process_message(message)
    ...
    if target_repo_value.empty?
      ...
      NeurolibreBookBuildWorker.perform_async(serializable(locals), target_repo_value, branch_name_value, context.issue_id, false)
    end
